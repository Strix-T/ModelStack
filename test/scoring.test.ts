...
describe("bundle ranking", () => {
  it("includes engine selection, variants, and concrete install steps on bundles", () => {
    const result = rankRecommendedBundles(getSeedCandidateCollections(), midSystem, minimalIntent());
    const bundle = result.bundles[0];
    expect(bundle?.recommendedEngine).toBeTruthy();
    expect(bundle?.fallbackEngine).toBeTruthy();
    expect(bundle?.nextSteps.length).toBeGreaterThan(0);
    expect(bundle?.stackArchetype).toBeTruthy();
  });
  ...
  it("produces bundles for a 16 GB unified-memory Mac profile", () => {
    const result = rankRecommendedBundles(getSeedCandidateCollections(), mac16System, minimalIntent());
  ...
  it("returns empty bundles with explanations when no text model fits memory constraints", () => {
    ...
    const result = rankRecommendedBundles(getSeedCandidateCollections(), tinyRam, minimalIntent());
  ...
  it("collectNoFitExplanations lists embedding gap when required but none fit", () => {
    ...
    const lines = collectNoFitExplanations(
      getSeedCandidateCollections(),
      tinyRam,
      minimalIntent({
        primaryUseCases: ["documents"],
